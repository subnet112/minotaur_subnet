...
class SolverBase:
    ...
    def setup_routes(self):
            ...
            try:
                ...
                def finalize_order():
                    ...
                    if order_id in self.orders:
                        ...
                        self.orders[order_id]['gas_used'] = random.randint(80000, 150000)
                
                threading.Thread(target=finalize_order, daemon=True).start()
                
                if self.logger:
                    self.logger.info(
                        f"Order accepted (Base, simulation): {order_id} | quote: {quote_id} | "
                        f"status: pending"
                    )
        ...
        def get_order(order_id):
            ...
            {
                "orderId": "order-id",
                "status": "pending" | "executed" | "settled" | "finalized" | "failed",
                "order": {
                    "createdAt": "2026-01-14T12:00:00Z",
                    "updatedAt": "2026-01-14T12:05:00Z",
                    "fillTransaction": {
                        "txHash": "0x...",
                        "blockNumber": 24232944,
                        "gasUsed": 81657
                    }
                }
            }
