...
5. **Weight emission**: the active champion's submitter receives **100% of miner emissions** (champion-takes-all; before a real miner champion exists, emissions burn to the subnet owner), committed on the tempo-aligned schedule (see [validator config](../validator/configuration.md#weight-emission)).
