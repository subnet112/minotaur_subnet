"""Regression guard: AnvilSimulator.simulate() must run the blocking web3
work OFF the asyncio event loop.
...
per-fork snapshot state). These tests lock that in.
"""
...
@pytest.mark.asyncio
async def test_simulate_does_not_block_the_event_loop():
    """A blocking _simulate_inner must NOT freeze the event loop.
...
@pytest.mark.asyncio
async def test_simulate_serializes_under_the_sim_lock():
