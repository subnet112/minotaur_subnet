...
challenger path: the round pin + the same _benchmark_submission call run_once
uses for a real challenger. This proves a king-clone is scored identically to
how the incumbent is scored (symmetry).
"""
...
def test_score_image_diagnostic_uses_challenger_path():
    w = _worker()
    ...
    async def _bench(image, ints, score_fn):
        seen["image"] = image
        return ["R1", "R2"]
    ...
    # benchmarked the GIVEN image via the challenger path (same as run_once)
