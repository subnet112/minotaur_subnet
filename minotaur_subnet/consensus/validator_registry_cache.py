...
def _chain_registry_env(chain_id: int) -> str:
    return os.environ.get(f"VALIDATOR_REGISTRY_{chain_id}", "").strip()


def _chain_rpc_env(chain_id: int) -> str:
    # Live-chain reads must hit the LIVE chain, never the sim fork. Prefer the
    # operator's *_UPSTREAM_RPC_URL (the live RPC they already supply for
    # Anvil's --fork-url), then fall back to the plain RPC. No hardcoded URLs.
    if chain_id == 8453:
