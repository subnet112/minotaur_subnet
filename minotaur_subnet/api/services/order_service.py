...
def build_swap_intent_params_hex(
    params: dict[str, Any],
    submitted_by: str,
) -> str | None:
    ...
    min_output = params.get("min_output_amount")
    missing = [
    ...
    if missing:
        logger.warning(
            "build_swap_intent_params_hex: missing required fields %s "
            "(order will fall back to manifest encoder, which produces an "
            "incomplete layout for DexAggregator._swap)", missing,
        )
        return None
    ...
    try:
        ...
        r_bytes = bytes.fromhex(permit_r.replace("0x", "").ljust(64, "0"))[:32]
        s_bytes = bytes.fromhex(permit_s.replace("0x", "").ljust(64, "0"))[:32]
        ...
        # behaviour of 1inch, CoW Swap, etc. — users virtually always want
        # native ETH, not the ERC-20 wrapper.
        from minotaur_subnet.blockchain.tokens import WRAPPED_NATIVE_TOKEN
